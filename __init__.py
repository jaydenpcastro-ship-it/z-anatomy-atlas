...
try:
    from . import zclassify
except ImportError:  # loaded outside a package (tests, scripts)
    import zclassify


BODY_SYSTEM_FILTERS = (
    ('ALL', 'All systems', 'Search every loaded anatomy system'),
    ('SKELETAL', 'Skeletal system', 'Bones, cartilages and sutures'),
    ('JOINTS', 'Joints', 'Joint capsules, ligaments and cavities'),
    ('MUSCULAR', 'Muscular system', 'Muscles, tendons, fascia and their origins/insertions'),
    ('FASCIA', 'Fascia & body envelope', 'Fascia shells that wrap the body (hide the muscles beneath)'),
    ('CARDIOVASCULAR', 'Cardiovascular system', 'Heart, arteries and veins'),
    ('LYMPHOID', 'Lymphoid organs', 'Lymph nodes, thymus, spleen, tonsils'),
    ('NERVOUS', 'Nervous system (all)', 'Central and peripheral nervous system and sense organs'),
    ('CNS', 'CNS', 'Central nervous system: brain, spinal cord, meninges'),
    ('PNS', 'PNS', 'Peripheral nervous system: nerves and ganglia'),
    ('SENSE', 'Sense organs', 'Eye and ear'),
    ('VISCERAL', 'Visceral systems', 'All viscera'),
    ('RESPIRATORY', 'Respiratory system', 'Nose, larynx, trachea, lungs'),
    ('DIGESTIVE', 'Digestive system', 'Mouth to anal canal, liver, bile ducts'),
    ('URINARY', 'Urinary system', 'Kidneys, ureters, bladder, urethra'),
    ('REPRODUCTIVE', 'Reproductive system', 'Genital organs'),
    ('ENDOCRINE', 'Endocrine glands', 'Hypophysis, thyroid, parathyroid...'),
    ('INTEGUMENTARY', 'Integumentary (hair)', 'Hairs and skin structures'),
    ('REGIONS', 'Regions of human body', 'Surface regions'),
    ('REFERENCE', 'Reference & movements', 'Reference lines, planes and movement arrows'),
)

JOINT_TYPE_FILTERS = (
    ('ALL', 'All joint types', 'Any joint type'),
    ('SYNOVIAL', 'Synovial', 'Synovial joints'),
    ('FIBROUS', 'Fibrous', 'Fibrous joints (sutures, syndesmoses)'),
    ('CARTILAGINOUS', 'Cartilaginous', 'Cartilaginous joints (symphyses, synchondroses)'),
    ('BALL_AND_SOCKET', 'Ball and socket', 'Hip, glenohumeral'),
    ('HINGE', 'Hinge', 'Knee, elbow, ankle, interphalangeal'),
    ('PIVOT', 'Pivot', 'Atlanto-axial, radio-ulnar'),
    ('SADDLE', 'Saddle', 'Sternoclavicular, thumb carpometacarpal'),
    ('PLANE', 'Plane', 'Acromioclavicular, intercarpal, intertarsal, zygapophysial...'),
    ('CONDYLOID', 'Condyloid', 'Radiocarpal, metacarpophalangeal, temporomandibular...'),
)

CARTILAGE_FILTERS = (
    ('ALL', 'All cartilage', 'Any cartilage'),
    ('HYALINE', 'Hyaline', 'Costal, articular, nasal, laryngeal and tracheal cartilage'),
    ('FIBROCARTILAGE', 'Fibrocartilage', 'Menisci, intervertebral discs, labra, articular discs'),
    ('ELASTIC', 'Elastic', 'Epiglottis, auricle, corniculate and cuneiform cartilage'),
)

_JOINT_KINDS = {'SYNOVIAL', 'FIBROUS', 'CARTILAGINOUS'}
_SYSTEM_ORDER = list(zclassify.TOP_SYSTEM.values()) + ['fascia', 'reference']
_nav_cache = {'key': None, 'records': []}


def _navigator_index(force=False):
    """Classified records for every structure/landmark. Built once per scene (7k objects takes ~a second),
    never on redraw; the panel's refresh button rebuilds it after edits."""
    key = (len(bpy.data.objects), len(bpy.data.collections))
    if force or _nav_cache['key'] != key:
        _nav_cache['records'] = zclassify.build_index(bpy.data.objects, bpy.context.scene.collection)
        _nav_cache['key'] = key
    return _nav_cache['records']


def _navigator_objects(props):
    """Records matching the panel filters, sorted as requested."""
    joint = props.navigator_joint_type
    kinds = ('structure', 'landmark') if props.navigator_landmarks else ('structure',)
    results = [
        rec for rec in _navigator_index()
        if zclassify.matches(
            rec,
            query=props.navigator_search.strip().casefold(),
            tag=props.navigator_system,
            joint_kind=joint if joint in _JOINT_KINDS else 'ALL',
            joint_type=joint if joint != 'ALL' and joint not in _JOINT_KINDS else 'ALL',
            cartilage=props.navigator_cartilage,
            kinds=kinds)
    ]
    query = props.navigator_search.strip().casefold()
    order = props.navigator_sort
    if order == 'NAME':
        key = lambda r: (r['name'].casefold(), r['side'])
    elif order == 'TYPE':
        key = lambda r: (r['kind'], r['name'].casefold(), r['side'])
    elif order == 'SYSTEM':
        key = lambda r: (_SYSTEM_ORDER.index(r['system']) if r['system'] in _SYSTEM_ORDER else 99,
                         r['path'], r['name'].casefold(), r['side'])
    else:
        key = lambda r: (r['kind'] != 'structure', r['name'].casefold(), r['side'])
    # a typed query always puts exact / prefix name matches first ("scapula" -> Scapula before Circumflex scapular artery)
    results.sort(key=lambda r: (zclassify.rank(r, query), key(r)))
    return results


def _reveal(obj, view_layer):
    """Make `obj` visible and selectable. The scene starts with systems 2-9 hidden at the *collection* level,
    where obj.hide_set(False) alone does nothing, so the numbered system collections holding it are shown too
    (never the excluded 'Bonus collection' tree)."""
    def walk(lc):
        yield lc
        for child in lc.children:
            yield from walk(child)

    numbered = {c.name for c in obj.users_collection if c.name[:1].isdigit() and c.name[1:2] == ':'}
    for lc in walk(view_layer.layer_collection):
...
class OBJECT_OT_navigator_select(bpy.types.Operator):
    """Select, reveal and frame one anatomy navigator result"""
    bl_idname = 'object.anatomy_navigator_select'
    bl_label = 'Select anatomy structure'
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        if obj is None:
            self.report({'WARNING'}, 'Structure is no longer available')
            return {'CANCELLED'}
        _reveal(obj, context.view_layer)
...
class OBJECT_OT_navigator_refresh(bpy.types.Operator):
    """Rebuild the navigator index (after renaming or adding objects)"""
    bl_idname = 'object.anatomy_navigator_refresh'
    bl_label = 'Refresh index'

    def execute(self, context):
        self.report({'INFO'}, f'{len(_navigator_index(force=True))} structures indexed')
        return {'FINISHED'}


class ZANATOMY_PT_Navigator(bpy.types.Panel):
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_label = 'Anatomy Navigator'
    ...
    def draw(self, context):
        props = context.scene.zanatomy
        layout = self.layout
        layout.prop(props, 'navigator_search', text='', icon='VIEWZOOM')
        layout.prop(props, 'navigator_system', text='System')
        row = layout.row(align=True)
        row.prop(props, 'navigator_joint_type', text='Joint')
        row.prop(props, 'navigator_cartilage', text='Cartilage')
        row = layout.row(align=True)
        row.prop(props, 'navigator_sort', text='Sort')
        row.operator(OBJECT_OT_navigator_refresh.bl_idname, text='', icon='FILE_REFRESH')
        layout.prop(props, 'navigator_landmarks')

        results = _navigator_objects(props)
        shown = min(len(results), self.MAX_ROWS)
        layout.label(text=f'{len(results)} found' + (f' (showing {shown}, refine the search)' if len(results) > shown else ''),
                     icon='OUTLINER_OB_MESH')
        if not results:
            layout.label(text='No matching structures')
            return
        col = layout.column(align=True)
        for rec in results[:shown]:
            label = rec['name'] + (f" ({rec['side'].upper()})" if rec['side'] else '')
            if len(label) > 36:
                label = label[:33] + '...'
            op = col.operator(OBJECT_OT_navigator_select.bl_idname, text=label,
                              icon='PINNED' if rec['kind'] == 'landmark' else 'OBJECT_DATA')
            op.object_name = rec['target']
fonts = {
   'English': 'Bfont',
    'Latin': 'Bfont',
    'Français': 'Bfont',
...
class ZAnatomyProps(bpy.types.PropertyGroup):
    ...
    navigator_search: bpy.props.StringProperty(
        default='',
        name='Search structures',
        description='Search object names, collections, and anatomy metadata',
    )
    navigator_system: bpy.props.EnumProperty(
        items=BODY_SYSTEM_FILTERS,
        default='ALL',
        name='System',
    )
    navigator_joint_type: bpy.props.EnumProperty(
        items=JOINT_TYPE_FILTERS,
        default='ALL',
        name='Joint type',
    )
    navigator_cartilage: bpy.props.EnumProperty(
        items=CARTILAGE_FILTERS,
        default='ALL',
        name='Cartilage',
    )
    navigator_landmarks: bpy.props.BoolProperty(
        default=True,
        name='Include landmarks',
        description='Also list landmarks (labelled points such as Acromion); selecting one frames the structure it sits on',
    )
    navigator_sort: bpy.props.EnumProperty(
        items=(
            ('NAME', 'Name', 'Alphabetical by structure name'),
            ('SYSTEM', 'System', 'Group by body system and anatomical group'),
            ('TYPE', 'Kind', 'Structures first, then landmarks'),
            ('MESH_FIRST', 'Structures first', 'Show real structures before landmarks'),
        ),
        default='MESH_FIRST',
        name='Sort results',
    )
...
classes = (
    OBJECT_OT_navigator_select,
    OBJECT_OT_navigator_refresh,
    ZANATOMY_PT_Navigator,
